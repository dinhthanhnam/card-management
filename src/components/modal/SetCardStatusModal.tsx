...
import {activateCard, setCardStatus} from "@/utils/activatecard";

interface SetCardStatusModalProps {
    cardContract?: Contract;
    onClose: () => void;
}

export default function SetCardStatusModal ({cardContract, onClose}: SetCardStatusModalProps)  {
    ...
    const handleSubmit = async () => {
        try {
            const response = await setCardStatus(payload);
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-10">
            ...
            >
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    <h2 className="text-xl font-bold mb-4 px-4">Thay Đổi Trạng Thái Thẻ {cardContract? `- ${cardContract.contractNumber}` : ""}</h2>
                    ...
                    <div className="flex-1 overflow-auto px-4">
                        ...
                        <div>
                            <FormInput
                                ...
                                onChange={(e) => setPayload({...payload, newStatus: e.target.value})}
                    ...
                    <div className="flex flex-row justify-end mt-6 px-4 py-2 border-t border-gray-200 bg-white">
                        <CommonButton onClick={handleSubmit}>Cập nhật</CommonButton>
                    </div>
                </div>
            </div>
        </div>
